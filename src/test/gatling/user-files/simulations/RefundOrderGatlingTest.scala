...
/**
 * Performance test for the RefundOrder entity.
 */
class RefundOrderGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the RefundOrder entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all refundOrders")
            .get("/api/refund-orders")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new refundOrder")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_refundOrder_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created refundOrder")
                .get("${new_refundOrder_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created refundOrder")
            .delete("${new_refundOrder_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
