...
export class CardHolderComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-card-holder div table .btn-danger'));
  title = element.all(by.css('jhi-card-holder div h2#page-heading span')).first();
...
export class CardHolderUpdatePage {
  pageTitle = element(by.id('jhi-card-holder-heading'));
  ...
  heldInput = element(by.id('field_held'));
  ...
  async setHeldInput(held: string): Promise<void> {
    await this.heldInput.sendKeys(held);
  }

  async getHeldInput(): Promise<string> {
    return await this.heldInput.getAttribute('value');
  }
...
export class CardHolderDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-cardHolder-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-cardHolder'));
