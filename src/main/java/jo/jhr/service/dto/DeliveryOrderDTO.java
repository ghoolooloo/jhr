...
public class DeliveryOrderDTO implements Serializable {
     ...
     */
    @ApiModelProperty(value = "配送状态（由配送平台提供，以达达为例）：1-4、8")
     ...
     */
    @ApiModelProperty(value = "配送违约金，单位：分。达达规定：接单后1－15分钟内取消订单，运费退回。同时扣除2元作为给配送员的违约金")
     ...
     */
    @NotNull
    @ApiModelProperty(value = "订单状态：DELIVERING（配送中）、COMPLETED（配送完成或自助取餐完成）", required = true)
    ...
    public boolean equals(Object o) {
        ...
        if (deliveryOrderDTO.getId() == null || getId() == null) {
            return false;
        }
        return Objects.equals(getId(), deliveryOrderDTO.getId());
    }
