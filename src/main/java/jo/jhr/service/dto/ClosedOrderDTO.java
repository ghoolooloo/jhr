...
/**
 * A DTO for the {@link jo.jhr.domain.ClosedOrder} entity.
 */
@ApiModel(description = "已完结订单\n@author Jo")
public class ClosedOrderDTO implements Serializable {
     ...
     */
    @ApiModelProperty(value = "配送状态（由配送平台提供，以达达为例）：4、5、7、10、1000")
     ...
     */
    @NotNull
    @ApiModelProperty(value = "订单状态：EXPIRED（已失效未支付）、REFUNDED（完成退款批准）、CLOSED（正常完结）", required = true)
    ...
    public boolean equals(Object o) {
        ...
        ClosedOrderDTO closedOrderDTO = (ClosedOrderDTO) o;
        if (closedOrderDTO.getId() == null || getId() == null) {
            return false;
        }
        return Objects.equals(getId(), closedOrderDTO.getId());
    }
